...
|
*/
// Base URL: prefer BASE_URL from .env when auto-detection is wrong (alternate port, reverse proxy, curl/CLI, subdirectory).
// On Hostinger / Cloudflare / nginx proxies, HTTPS may only appear in X-Forwarded-Proto or REQUEST_SCHEME.
$_port = isset($_SERVER['SERVER_PORT']) ? (int) $_SERVER['SERVER_PORT'] : 80;
$_https_raw = isset($_SERVER['HTTPS']) ? strtolower((string) $_SERVER['HTTPS']) : '';
$_is_https = ($_https_raw !== '' && $_https_raw !== 'off')
    || $_port === 443
    || (isset($_SERVER['REQUEST_SCHEME']) && strtolower((string) $_SERVER['REQUEST_SCHEME']) === 'https')
    || (isset($_SERVER['HTTP_X_FORWARDED_PROTO']) && strtolower((string) $_SERVER['HTTP_X_FORWARDED_PROTO']) === 'https')
    || (isset($_SERVER['HTTP_X_FORWARDED_SSL']) && (string) $_SERVER['HTTP_X_FORWARDED_SSL'] === '1')
...
$base_url_override = getenv('BASE_URL');
if (($base_url_override === false || $base_url_override === '') && isset($_ENV['BASE_URL'])) {
    $base_url_override = (string) $_ENV['BASE_URL'];
}
// Some hosts do not mirror putenv() into getenv(); index.php also sets $_SERVER['BASE_URL'] from .env.
if (($base_url_override === false || $base_url_override === '') && isset($_SERVER['BASE_URL'])) {
    $base_url_override = (string) $_SERVER['BASE_URL'];
}
$base_url_override = is_string($base_url_override) ? trim($base_url_override) : '';
if ($base_url_override !== '') {
    $config['base_url'] = rtrim($base_url_override, '/') . '/';
} else {
...
if ($google_maps_api_key === false || $google_maps_api_key === '') {
    $google_maps_api_key = isset($_SERVER['GOOGLE_MAPS_API_KEY']) ? (string) $_SERVER['GOOGLE_MAPS_API_KEY'] : '';
}
if ($google_maps_api_key === '') {
    $local_gmaps = APPPATH . 'config/google_maps_api_key.local.php';
    if (is_file($local_gmaps)) {
        $from_local = include $local_gmaps;
        if (is_string($from_local) && $from_local !== '') {
            $google_maps_api_key = $from_local;
        }
    }
}
$config['google_maps_api_key'] = $google_maps_api_key;
$config['nb_admin_email'] = getenv('NB_ADMIN_EMAIL') ?: 'admin@dreamvillamakers.com';
...
*/
// Set TRUE when legacy POST routes are updated to send CSRF tokens (or add uri patterns to csrf_exclude_uris).
