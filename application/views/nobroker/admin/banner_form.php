...
<div class="nb-admin-page-head d-flex flex-wrap justify-content-between align-items-start gap-3">
  <div>
    ...
    <p class="nb-admin-page-desc mb-0">Upload a wide hero image for the homepage slideshow. Active banners appear on the home page only.</p>
  </div>
...
<div class="nb-admin-panel">
  ...
  <form method="post" enctype="multipart/form-data" class="row g-3">
    <div class="col-md-6">
      ...
      <?php $status = isset($row->status) ? strtolower((string) $row->status) : 'active'; ?>
    ...
    <?php if (!empty($row)) : ?>
      ...
      if (isset($row->imageUrl) && trim((string) $row->imageUrl) !== '') {
          $cur = trim((string) $row->imageUrl);
      } elseif (isset($row->image) && trim((string) $row->image) !== '') {
          $cur = trim((string) $row->image);
      }
      ?>
      <?php if ($cur !== '') : ?>
      <?php $imgUrl = preg_match('/^https?:\/\//i', $cur) ? $cur : base_url($cur); ?>
      <div class="col-12">
        <label class="form-label">Current image</label><br>
        <img src="<?php echo html_escape($imgUrl); ?>" alt="" style="max-width:320px;max-height:160px;object-fit:cover;border-radius:8px;border:1px solid #dbe1ea;">
      </div>
      <?php endif; ?>
    <?php endif; ?>
    <div class="col-12 d-flex gap-2">
      <button type="submit" class="btn btn-primary rounded-pill px-4"><?php echo $isEdit ? 'Update' : 'Create'; ?></button>
      <a class="btn btn-outline-secondary rounded-pill px-4" href="<?php echo site_url('panel/banners'); ?>">Cancel</a>
    </div>
  </form>
  </div>
</div>
